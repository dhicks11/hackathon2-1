...
export default function IdeasPage() {
  ...
  useEffect(() => {
    api.ideas.list().then(d => setIdeas(Array.isArray(d) ? d : [])).finally(() => setLoading(false))
  }, [])
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ background:'#F8F8F8', borderBottom:'1px solid #E6E6E6' }}>
                {['Idea','Category','Score','Status',''].map(h => (
              ...
              {ideas.map((idea:any, i:number) => {
                ...
                return (
                  <tr key={idea.id} style={{ borderBottom: i < ideas.length-1 ? '1px solid #F2F2F2' : 'none' }}>
                    ...
                    <td style={{ padding:'14px 16px', fontSize:12, color:'#999' }}>{idea.category || '—'}</td>
                    <td style={{ padding:'14px 16px', fontFamily:'IBM Plex Mono', fontSize:13, color: idea.score>=7?'#00875A':idea.score>=4?'#FF8800':'#999' }}>
                      {idea.score ? `${idea.score}/10` : '—'}
                    </td>
                    <td style={{ padding:'14px 16px' }}>
