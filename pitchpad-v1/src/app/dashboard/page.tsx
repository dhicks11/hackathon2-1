...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.metrics.get(), api.ideas.list()])
      .then(([m, i]) => { setMetrics(m); setIdeas(Array.isArray(i) ? i.slice(0, 6) : []) })
  ...
  const STATUS_COLORS: Record<string, { bg: string; color: string }> = {
    ...
    reviewing:  { bg: '#EBF3FF', color: '#0066CC' },
    ...
    <div className="space-y-8">
      <div className="anim-fade-up">
        ...
        <h1 style={{ fontSize: 28, fontWeight: 300, color: '#111', letterSpacing: '-0.02em' }}>
          {user?.role === 'reviewer' ? 'Review Dashboard' : 'Your workspace'}
        </h1>
        <p style={{ color: '#999', fontSize: 13, marginTop: 4 }}>{user?.email}</p>
      </div>
      ...
      {metrics && (
        <div className="grid grid-cols-3 gap-4 anim-fade-up anim-stagger-1">
          {[
            { label: 'Total Ideas',      value: metrics.total_ideas      ?? 0, accent: true  },
            { label: 'Total Feedback',   value: metrics.total_feedback   ?? 0, accent: false },
            { label: 'Pitch Attempts',   value: metrics.pitch_attempts   ?? 0, accent: false },
            { label: 'Avg Idea Score',   value: metrics.avg_idea_score   ?? '—', accent: false },
            { label: 'Promising Ideas',  value: metrics.promising_ideas  ?? 0, accent: false },
          ].map(s => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 anim-fade-up anim-stagger-2">
          ...
          <div className="lv-card overflow-hidden">
            {ideas.length === 0 ? (
              <div style={{ padding: 40, textAlign: 'center', color: '#999', fontSize: 13 }}>No ideas yet</div>
            ) : (
                  ...
                  {ideas.map((idea: any, i: number) => {
                    ...
                    return (
                      <tr key={idea.id} style={{ borderBottom: i < ideas.length - 1 ? '1px solid #F2F2F2' : 'none' }}>
                        ...
                        <td style={{ padding: '12px 16px', fontFamily: 'IBM Plex Mono', fontSize: 13, color: idea.score >= 7 ? '#00875A' : idea.score >= 4 ? '#FF8800' : '#999' }}>
                          {idea.score ?? '—'}/10
                        </td>
                        <td style={{ padding: '12px 16px' }}>
        ...
        <div className="space-y-3 anim-fade-up anim-stagger-3">
          ...
          {[
            { href: '/ideas/new',   label: 'New Idea',      sub: '6-step submission',      bg: '#FFF0F2', color: '#E2001A', icon: '+' },
            { href: '/practice',    label: 'Practice Pitch',sub: 'AI voice coaching',       bg: '#EBF3FF', color: '#0066CC', icon: '◎' },
            { href: '/ai-assistant',label: 'AI Assistant',  sub: 'Ask anything',           bg: '#E6F9F1', color: '#00875A', icon: '⚡' },
            { href: '/export',      label: 'Export Deck',   sub: 'Download .pptx',         bg: '#F8F8F8', color: '#666',    icon: '↓' },
          ].map(a => (
