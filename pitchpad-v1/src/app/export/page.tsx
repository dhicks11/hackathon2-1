...
export default function ExportPage() {
  ...
  async function exportAs(format: 'pptx' | 'pdf') {
    ...
    try {
      ...
      a.href = url; a.download = `pitch-${selected}.${format}`; a.click()
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      <div className="anim-fade-up">
        ...
        <p style={{ color: '#999', fontSize: 13, marginTop: 4 }}>Generate and share your pitch deck</p>
      </div>

      {/* Idea selector + actions */}
      ...
      {generating && (
        <div className="lv-card p-12" style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: '#999', fontSize: 13 }}>Claude is building your pitch deck…</p>
        </div>
      )}
      ...
      {!generating && deck && (
        <div className="anim-fade-up anim-stagger-2">
          <p style={{ fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.07em', color: '#999', marginBottom: 12 }}>
            {deck.length} slides · Click a slide to expand
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
            {deck.map((slide, i) => {
              const accentColor = SLIDE_TYPE_COLORS[slide.type] ?? '#333'
              return (
                <div key={slide.id} className="lv-card-hover p-5" style={{ position: 'relative', overflow: 'hidden' }}>
                  {/* Slide number */}
      ...
      {!generating && !deck && selected && (
        <div className="lv-card p-12" style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: '#CCC', fontSize: 12 }}>Click "Generate Deck" to let Claude build your investor-ready pitch summary</p>
        </div>
      )}
